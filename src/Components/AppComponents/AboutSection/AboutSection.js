   import AnimationsProvider from '../../Service/AnimationsProvider/AnimationsProvider.js';
   ...
   export default class AboutSection extends HTMLElement {
      ...
      async init() {
         ...
         sectionOrder.forEach((section, idx) => {
            if (section && section instanceof Node) {
               AnimationsProvider.fadeIn(section, { duration: 800, delay: 400 + idx * 200, translateY: ['40px', '0px'] });
            }
         });
      ...
      async setupEnhancedHero() {
         ...
         AnimationsProvider.fadeIn(profileContainer, { duration: 900, delay: 100 });
         AnimationsProvider.slideInLeft(heroText, { duration: 900, delay: 300 });

         // Animar los botones de CTA
         [contactBtn, projectsBtn, cvBtn].forEach((btn, idx) => {
            AnimationsProvider.fadeIn(btn, { duration: 700, delay: 600 + idx * 120, translateY: ['20px', '0px'] });
         });
      ...
      async createEnhancedAboutCards() {
         ...
         setTimeout(() => {
            ...
            cards.forEach((card, idx) => {
               AnimationsProvider.slideInLeft(card, { duration: 700, delay: 100 + idx * 180 });
            });
         }, 100);
      }
      ...
      async createCarruselSection() {
         ...
         setTimeout(() => {
            AnimationsProvider.slideInLeft(valuesTitle, { duration: 800, delay: 100 });
         }, 100);
